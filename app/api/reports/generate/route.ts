...
export async function POST(req: NextRequest) {
  ...
  const { reportType, department, employeeId } = await req.json();
  ...
  await connectDB();
  try {
    const report = await generateAndSendReport(reportType, department || undefined, user!._id.toString(), "current", employeeId || undefined);
