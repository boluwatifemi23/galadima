...
export default function ReportsPage() {
  ...
  const [loading, setLoading] = useState(true);
   const [generateForm, setGenerateForm] = useState({ reportType: "weekly", department: "", employeeId: "" });
  const [generating, setGenerating] = useState(false);
 const [staffOptions, setStaffOptions] = useState<StaffOption[]>([]);``

  useEffect(() => {
    const params = new URLSearchParams({ role: "staff" });
    if (generateForm.department) params.set("department", generateForm.department);
    fetch(`/api/users?${params.toString()}`).then((res) => res.json()).then((json) => json.success && setStaffOptions(json.users));
 ...
 const [viewing, setViewing] = useState<ReportHistoryItem | null>(null);
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
        ...
        body: JSON.stringify({ reportType: generateForm.reportType, department: generateForm.department || undefined, employeeId: generateForm.employeeId || undefined }),
      });
      const json = await res.json();
      if (!json.success) {
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <form onSubmit={handleGenerate} className="card-body form-grid-2">
         ...
         <div className="form-group">
            ...
            <select
             title="Department"
             className="form-select" value={generateForm.department} onChange={(e) => setGenerateForm({ ...generateForm, department: e.target.value, employeeId: "" })}>
          ...
          <div className="form-group">
            ...
            <select
             title="Staff"
             className="form-select" value={generateForm.employeeId} onChange={(e) => setGenerateForm({ ...generateForm, employeeId: e.target.value })}>
              <option value="">{generateForm.department ? `Everyone in ${generateForm.department}` : "Whole company"}</option>
              {staffOptions.map((s) => <option key={s._id} value={s._id}>{s.name}</option>)}
            </select>
            <p className="form-hint">Choosing a person here overrides the department above — it generates a report for them specifically.</p>
          </div>
        ...
        <div className="card-body" style={{ padding: 0 }}>
          {loading ? (
            ...
            <table className="data-table">
               <thead><tr><th>Type</th><th>Scope</th><th>Period</th><th>Recipients</th><th>Generated By</th><th>Generated On</th><th></th></tr></thead>
              <tbody>
                {history.map((r) => (
                  <tr key={r._id}>
                    <td style={{ textTransform: "capitalize" }}>{r.reportType}</td>
                    <td>{r.employeeName || r.department || "Whole Company"}</td>
